...
import { timeAgo } from "../events/EventUI";
            ...
            <Popup closeButton={false}>
              <div className="font-inherit min-w-[180px] text-[0.78rem] leading-normal">
                <div className="mb-1 text-[0.85rem] font-bold" style={{ color }}>
                  ...
                  {q.tsunami_alert && <span className="ml-1.5 text-[0.7rem] text-app-accent-red">⚠ {t.earthquakeTsunami}</span>}
                </div>
                <div className="mb-0.5 text-app-text-body-dim">{q.location_name}</div>
                {q.depth_km != null && (
                  <div className="text-[0.7rem] text-app-text-secondary">{t.earthquakeDepth} {q.depth_km.toFixed(0)} km</div>
                )}
                <div className="mt-1 text-[0.68rem] text-app-text-muted">{timeAgo(q.occurred_at, lang)}</div>
              </div>
            </Popup>
          </CircleMarker>
        );
