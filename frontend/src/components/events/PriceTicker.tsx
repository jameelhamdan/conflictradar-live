...
function PriceRow({ tick, flash }: PriceRowProps) {
  ...
  return (
    ...
    >
      <span style={{ flex: '0 0 72px', fontSize: '0.72rem', fontWeight: 600, color: '#c8c8d8', letterSpacing: '0.01em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {tick.symbol}
      </span>
      <span style={{ flex: 1, fontSize: '0.68rem', color: '#55556a', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {tick.name}
      </span>
      <span style={{ flex: '0 0 80px', textAlign: 'right', fontSize: '0.76rem', fontWeight: 600, color: '#e0e0ec', fontVariantNumeric: 'tabular-nums' }}>
        {formatValue(tick.value, tick.stream_key)}
      </span>
      <span style={{ flex: '0 0 56px', textAlign: 'right', fontSize: '0.7rem', fontWeight: 500, color, fontVariantNumeric: 'tabular-nums' }}>
        {tick.change_pct != null ? `${tick.change_pct >= 0 ? '+' : ''}${tick.change_pct.toFixed(2)}%` : '—'}
      </span>
    </div>
  )
}
...
interface PriceTickerProps {
  /** Latest SSE price_tick events (from useSSE in App) */
  latestTick: { symbol: string; value: number; change_pct: number | null; occurred_at: string } | null
}
...
export default function PriceTicker({ latestTick }: PriceTickerProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', borderBottom: '1px solid #1e1e2a' }}>
      {/* Header + tabs */}
          ...
          {STREAM_KEYS.map(key => (
            <button
              ...
              style={{
                ...
                borderRadius: 99,
                border: `1px solid ${activeKey === key ? '#7c9ef855' : 'transparent'}`,
